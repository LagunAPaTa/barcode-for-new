...
class PluginBarcodeBarcode {
   ...
   function printPDF($p_params) {
      ...
      // Support for Chinese and other UTF-8 characters
      // Try to use a Unicode-capable font, fallback to Helvetica
      $fontPath = Plugin::getPhpDir('barcode')."/lib/ezpdf/fonts/Helvetica.afm";
            ...
            if (file_exists($imgFile)) {
               ...
               for ($i = 0; $i < count($displayData); $i++) {
                   ...
                   $pdf->addTextWrap(
                       ...
                       $width,
                       'center');
                   $txtHeight += $txtSpacing/2 + $fontSize;
               }
               if ($p_params['border']) {
